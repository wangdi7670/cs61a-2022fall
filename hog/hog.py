...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for i in range(num_rolls):
        current_turn_score = dice()

        if current_turn_score == 1:
            is_SowSad = True
        else:
            sum += current_turn_score
...
# 返回的是那个回合的得分，不是回合结束后的最终分数，最终分数 = 当前分数 + 当前回合的得分
...
def oink_points_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    zero_dice_score = oink_points(score, opponent_score)
    if zero_dice_score >= threshold:
        return 0
    else:
        return num_rolls
...
def pigs_on_prime_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    if pigs_on_prime(score + oink_score, opponent_score):
        return 0
    else:
        return oink_points_strategy(score, opponent_score, threshold=threshold, num_rolls=num_rolls)
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    return oink_points_strategy(score, opponent_score, threshold=10, num_rolls=3)
