...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    split("sp'lowfOot ploW&fO.oT -{ploWfootL monOciliaTedp yplowfootA")
    "*** YOUR CODE HERE ***"
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    while index < len(word_list):
        if word_list[index] == typed_word:
            ...
            if index == 0:
                diff = diff_function(typed_word, word_list[index], limit)
                lowest_diff_word = word_list[index]
            else:
                temp = diff_function(typed_word, word_list[index], limit)
                if temp < diff:
                    lowest_diff_word = word_list[index]
                    diff = temp
        index += 1
...
def feline_fixes(typed, reference, limit):
    ...
    if typed[0] != reference[0]:
        count = 1
    return feline_fixes(typed[1:], reference[1:], limit) + count
